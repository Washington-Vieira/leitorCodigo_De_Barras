...
def abrir_resumo_status(status):
    ...
    colunas = ("Pedido", "Máquina", "Item", "Status", "Qtd Programada", "Qtd Enviada", "Diferença")
    ...
    def carregar_dados(filtro=""):
        ...
        query = '''
            SELECT p.pedido, p.maquina, p.item, p.nome_status,
                   SUM(p.quantidade) as qtd_programada,
                   COALESCE((SELECT COUNT(*) FROM leituras l WHERE l.pedido_lido = p.pedido AND l.serial IN (SELECT serial FROM produtos WHERE item = p.item AND maquina = p.maquina)), 0) as qtd_enviada
            FROM produtos p
            WHERE p.nome_status = ?
            GROUP BY p.pedido, p.maquina, p.item, p.nome_status
        '''

        cursor.execute(query, (status,))
        for row in cursor.fetchall():
            pedido, maquina, item, status_val, qtd_prog, qtd_env = row
            diff = qtd_prog - qtd_env
            if filtro.lower() in pedido.lower() or filtro.lower() in item.lower():
                tree.insert("", tk.END, values=(pedido, maquina, item, status_val, qtd_prog, qtd_env, diff))
...
def leitor_codigo_barras_gui():
        ...
        if resultado:
            ...
            if not ja_lido:
                ...
                cursor.execute('''
                    ...
                    WHERE l.pedido_lido = ? AND l.serial IN (
                        SELECT serial FROM produtos WHERE item = ? AND maquina = ?
                    )
                ''', (pedido, item, maquina))
                qtd_enviada = cursor.fetchone()[0]

                cursor.execute('''
                    SELECT SUM(quantidade) FROM produtos
                    WHERE pedido = ? AND item = ? AND maquina = ?
                ''', (pedido, item, maquina))
                qtd_programada = cursor.fetchone()[0] or 0

                tree.insert("", tk.END, values=(pedido, serial, item, maquina, linha_lida, quantidade, status, data_leitura, qtd_programada, qtd_enviada))
            else:
    ...
    colunas = ("Pedido", "Serial", "Item", "Máquina", "Linha", "Quantidade", "Status", "Data/Hora Leitura", "Qtd Programada", "Qtd Enviada")
    ...
    cursor.execute('''
        SELECT l.pedido_lido, l.serial, p.item, p.maquina, l.linha_lida, p.quantidade, p.nome_status, l.data_leitura,
               (SELECT SUM(quantidade) FROM produtos WHERE pedido = p.pedido AND item = p.item AND maquina = p.maquina) as qtd_programada,
               (SELECT COUNT(*) FROM leituras l2 WHERE l2.pedido_lido = l.pedido_lido AND l2.serial IN (
                   SELECT serial FROM produtos WHERE item = p.item AND maquina = p.maquina
               )) as qtd_enviada
        FROM leituras l
