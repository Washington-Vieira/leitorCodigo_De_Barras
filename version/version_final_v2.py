...
def abrir_resumo_status(status):
    ...
    colunas = ("Pedido", "Data Pedido", "Máquina", "Item", "Status", "Qtd Programada", "Qtd Enviada", "Diferença")
    ...
    def carregar_dados(filtro=""):
        ...
        for row in cursor.fetchall():
            pedido, data_pedido, maquina, item, status_val, qtd_prog, qtd_env = row
            diff = qtd_prog - qtd_env
            if filtro.lower() in pedido.lower() or filtro.lower() in item.lower():
                tree.insert("", tk.END, values=(pedido, data_pedido, maquina, item, status_val, qtd_prog, qtd_env, diff))
...
def leitor_codigo_barras_gui():
    ...
    colunas = ("Pedido", "Data Pedido", "Serial", "Item", "Máquina", "Linha", "Qtd Programada", "Qtd Enviada", "Status", "Data/Hora Leitura")
