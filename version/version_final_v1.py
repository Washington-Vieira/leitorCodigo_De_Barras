...
def leitor_codigo_barras_gui():
    def verificar_codigo(event=None):
        ...
        if resultado:
            ...
            if not ja_lido:
                ...
                tree.insert("", tk.END, values=(pedido, serial, item, maquina, linha_lida, quantidade, qtd_enviada, status, data_leitura))
            else:
    ...
    colunas = ("Pedido", "Serial", "Item", "Máquina", "Linha", "Qtd Programada", "Qtd Enviada", "Status", "Data/Hora Leitura")
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT l.pedido_lido, l.serial, p.item, p.maquina, l.linha_lida,
               p.quantidade, (SELECT SUM(pp.quantidade) FROM produtos pp WHERE pp.serial = l.serial),
               p.nome_status, l.data_leitura
        FROM leituras l
