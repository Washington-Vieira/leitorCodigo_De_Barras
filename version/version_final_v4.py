...
def abrir_resumo_status(status):
    ...
    botao_exportar = tk.Button(frame_top, text="📤 Exportar para Excel", command=lambda: exportar_para_excel(tree, f"resumo_{status.lower()}.xlsx"))
...
def exportar_para_excel(tree, nome_arquivo_default):
    # Obter a data e o horário atuais
    data_hora_atual = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    
    # Adicionar a data e o horário ao nome do arquivo padrão
    nome_arquivo_default = f"{nome_arquivo_default.split('.')[0]}_{data_hora_atual}.xlsx"

    # Abrir janela para o usuário escolher o diretório e nome do arquivo
    nome_arquivo = asksaveasfilename(
        defaultextension=".xlsx",
        filetypes=[("Arquivos Excel", "*.xlsx"), ("Todos os arquivos", "*.*")],
        initialfile=nome_arquivo_default,
    ...
    messagebox.showinfo("Exportação Concluída", f"Os dados foram exportados para o arquivo '{nome_arquivo}'.")
...
def leitor_codigo_barras_gui():
    ...
    botao_exportar = tk.Button(top_frame, text="📤 Exportar para Excel", command=lambda: exportar_para_excel(tree, "dados_exportados.xlsx"))
    botao_exportar.pack(side="left", padx=5)
